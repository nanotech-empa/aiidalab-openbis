...
def create_project_in_openbis(
    session, space_code: str, project_code: str, project_info: dict
):
    try:
        session.new_project(
            space=space_code, code=project_code, description=project_info["description"]
        ).save()
    except ValueError:
        print(f"Project {project_code} exists already.")


def create_experiment_in_openbis(
    session,
    space_code: str,
    project_code: str,
    experiment_code: str,
    ...
    try:
        experiment = session.new_experiment(
            ...
            type="COLLECTION",
            project=f"/{space_code}/{project_code}/",
        )
...
def create_experiment_type_in_openbis(session):
    try:
        experiment_type = session.new_experiment_type(
            "EXPERIMENT", description="Experiment"
        )
        experiment_type.save()
        experiment_type.assign_property(prop="name", section="General info")
        experiment_type.assign_property(
            prop="default_collection_view", section="General info"
        )
    except ValueError:
        print("Experiment type EXPERIMENT exists already.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/home/jovyan/apps/aiidalab-openbis/config/openbis_config.json",
    )
    ...
    print(
        f"Running setup on URL: {openbis_url} with the token: {openbis_token} and using the config file {config_filepath}"
    )
    ...
    if session and config_file:
        ...
        openbis_classes = [
            cls
            for name, cls in inspect.getmembers(openbis_objects, inspect.isclass)
        ...
        for cls in tqdm(openbis_classes):
            if issubclass(cls, BaseModel) and hasattr(cls, "get_code"):
                ...
                object_type_dict = {
                    "generatedCodePrefix": cls.get_code(),
                    "code": object_code,
                }
                ...
                if object_code.upper() in ["MEASUREMENT_SESSION", "STM_SIMULATION"]:
                    try:
                        ...
                        assign_property_to_object_type_in_openbis(
                            session, object_type, property_type
                        )
                    except ValueError:
                        print("Property type default_object_view does not exist.")
                ...
                for name, field in cls.model_fields.items():
                    ...
                    if field_metadata:
                        ...
                        if field_datatype in [
                            ...
                            if "custom_widget" in field_metadata:
                                ...
                                property_type_dict["metaData"] = {
                                    "custom_widget": field_custom_widget
                                }
                            ...
                            property_type_dict["dataType"] = field_datatype
                            enum_name, enum_keys, enum_values = extract_enum_info(
                                field.annotation
                            )
                            ...
                            # Create vocabulary
                            vocabulary = create_vocabulary_in_openbis(
                                session, vocabulary_dict
                            )
                    ...
                    if create_property_type:
                        ...
                        property_type = create_property_type_in_openbis(
                            session, property_type_dict
                        )
                        assign_property_to_object_type_in_openbis(
                            session, object_type, property_type
                        )
        ...
        documentation_website_property = session.get_property_types(
        ...
        for space_code, space_info in collections_config["spaces"].items():
            ...
            if "projects" in space_info:
                for project_code, project_info in space_info["projects"].items():
                    create_project_in_openbis(
                        session, space_code, project_code, project_info
                    )

                    # Create experiments in openBIS
                    if "experiments" in project_info:
                        for experiment_code, experiment_info in project_info[
                            "experiments"
                        ].items():
                            create_experiment_in_openbis(
                                session,
                                space_code,
                                project_code,
                                experiment_code,
