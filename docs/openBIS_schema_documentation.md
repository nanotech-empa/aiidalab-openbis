...
1. [Object Types](#1-object-types): Defines the core entities within the laboratory and simulation ecosystem. These encompass physical materials (e.g., `2D_LAYER_MATERIAL`, `CRYSTAL`, `SUBSTANCE`), instrument setups and hardware components (e.g., `INSTRUMENT.STM`, `TURBO_PUMP`, `SPUTTER_GUN`), discrete experimental procedures and operational logbook entries (e.g., `ANNEALING`, `DEGASING_LOGENTRY`, `SPUTTERING`), and computational nodes or structures (e.g., `ATOMISTIC_MODEL`, `AIIDA_NODE`, `SPM_SIMULATION`).
...
* The normative target schema defines the following object types:
...
The superseded simulation types `PDOS`, `POTENTIAL_ENERGY_CALCULATION`, and
`STM_SIMULATION` may still exist on deployed servers but are not part of this
target schema. Projected density of states is represented by `DOS` with its
`PDOS` property, potential-energy results by `ENERGY_CALCULATION`, and simulated
probe microscopy by `SPM_SIMULATION`.
...
| `url` | URL | URL | HYPERLINK | False | False | False |
...
| `description` | Description | Description | VARCHAR | False | False | False |
| `comments` | Comments | Comments | VARCHAR | False | False | False |
| `code` | Code | Code | OBJECT (CODE) | False | False | False |
| `computer` | Computer | Computer | OBJECT (COMPUTER) | False | False | False |
...
* The normative target schema defines the following dataset types:
...
* The normative target schema defines the following vocabulary types:
